...
from MiV import cells, synapses, utils, neuron_utils, io_utils
from MiV.env import Env
from MiV.synapses import get_syn_filter_dict
from MiV.utils import Context, get_module_logger, is_interactive, config_logging
from MiV.neuron_utils import h, configure_hoc_env, make_rec, run_iclamp
...
def init_biophys_cell(env, pop_name, gid, load_weights=True, load_connections=True, register_cell=True, write_cell=False, validate_tree=True, cell_dict={}):
    ...
    elif if template_name.lower() == 'sc_nrn':
        cell = cells.make_SC_cell(env, pop_name, gid,
                                  tree_dict=cell_dict.get('morph', None),
                                  synapses_dict=cell_dict.get('synapse', None),
                                  connection_graph=cell_dict.get('connectivity', None),
                                  weight_dict=cell_dict.get('weight', None),
