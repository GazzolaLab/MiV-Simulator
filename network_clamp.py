...
import numpy as np
import click
from MiV import io_utils, spikedata, synapses, stimulus, cell_clamp
from MiV.cells import h, make_input_cell, register_cell, record_cell, report_topology, is_cell_registered, load_biophys_cell_dicts
from MiV.env import Env
from MiV.neuron_utils import h, configure_hoc_env
from MiV.utils import is_interactive, is_iterable, Context, list_find, list_index, viewitems, zip_longest, get_module_logger, config_logging, generate_results_file_id
from MiV.utils import write_to_yaml, read_from_yaml, get_trial_time_indices, get_trial_time_ranges, get_low_pass_filtered_trace, contiguous_ranges
from MiV.cell_clamp import init_biophys_cell
from MiV.stimulus import rate_maps_from_features, oscillation_phase_mod_config
...
def init_inputs_from_spikes(env, presyn_sources, time_range,
    ...
    for population in populations:
        ...
        logger.info(f"Initializing {len(gid_range)} spike sources for population {population}.")
...
def optimize_run(env, pop_name, param_config_name, init_objfun, problem_regime, nprocs_per_worker=1,
    ...
    problem_metadata = np.array([tuple(( opt_targets[k] for k in sorted(opt_targets) if isinstance(opt_targets[k], float) ))],
                                dtype=[(f'Target {k}', np.float32, (1,)) for k in sorted(opt_targets)  if isinstance(opt_targets[k], float)])
...
def dist_ctrl(controller, init_params, cell_index_set, param_path, pop_param_tuple_dicts):
    ...
    if len(param_path) > 0:
        for this_param_path, pop_param_tuple_dict in zip(param_path, pop_param_tuple_dicts):
            ...
            task_id = controller.submit_call("dist_run", module_name="MiV.network_clamp",
...
@click.option("--params-path", required=False, multiple=True, type=click.Path(exists=True, file_okay=True, dir_okay=False), \
              help='optional path to parameters generated by optimize')
@click.option("--params-id", required=False, multiple=True, type=int, \
...
def go(config_file, population, dt, gids, gid_selection_file, arena_id, trajectory_id, generate_weights, t_max, t_min,
       ...
       spike_events_path, spike_events_namespace, spike_events_t,
       coords_path, distances_namespace, phase_mod,
       input_features_path, input_features_namespaces, n_trials, params_path, params_id,
    ...
    if size > 1:
        ...
        if distwq.is_controller:
            distwq.run(fun_name="dist_ctrl", module_name="MiV.network_clamp",
