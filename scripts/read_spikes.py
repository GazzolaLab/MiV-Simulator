...
def read_population_ranges(file_path):
    with h5py.File(file_path, 'r') as f:
        population_labels = f[grp_h5types][dtype_population_labels].dtype.metadata['enum']
...
def read_cell_attributes(file_path, population, namespace):
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        if population not in f[grp_populations]:
            raise RuntimeError(f'Population {population} not found in Populations group in file {file_path}')
        if namespace not in f[grp_populations][population]:
            raise RuntimeError(f'Namespace {namespace} for population {population} not found in file {file_path}')
