...
import yaml
import MiV
from MiV import utils, io_utils
from MiV.env import Env
from neuroh5.io import read_cell_attributes, read_cell_attribute_selection, read_graph_selection, read_population_ranges, append_cell_attributes
import h5py
...
@click.option("--input-features-path", required=False, type=click.Path(exists=True, file_okay=True, dir_okay=False))
@click.option("--input-features-namespaces", type=str, required=False, multiple=True, default=[])
...
@click.option("--stimulus-id", required=True, type=str,
              help='name of stimulus used')
@click.option("--verbose", "-v", is_flag=True)
def main(arena_id, config, config_prefix, dataset_prefix, distances_namespace, spike_input_path, spike_input_namespace, spike_input_attr, input_features_namespaces, input_features_path, selection_path, output_path, io_size, stimulus_id, verbose):
    ...
    env = Env(comm=comm, config_file=config, 
              ...
              arena_id=arena_id, stimulus_id=stimulus_id, io_size=io_size)
